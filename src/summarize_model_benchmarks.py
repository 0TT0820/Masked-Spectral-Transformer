"""Summarize the selected Raman benchmark model families.
...
def main() -> None:
    ...
    selected.to_csv(OUT_DIR / "validation_selected_summary.csv", index=False, encoding="utf-8-sig")
    ...
    best_grid.to_csv(OUT_DIR / "best_observed_grid_summary.csv", index=False, encoding="utf-8-sig")

    md = [
        "# Benchmark Model Summary",
        "",
        "This summary focuses on the model families used in the main open benchmark: PCA-SVM, PLS-DA, 1D-CNN, Standard Transformer, and MST.",
        "",
    ...
    (OUT_DIR / "baseline_model_summary.md").write_text("\n".join(md), encoding="utf-8")
