...
OUT_DIR = ROOT / "data" / "overview" / "data_inventory"
...
def class_label(category: str) -> str:
...
def main() -> None:
    ...
    parent["class_label"] = parent["major_category"].map(class_label)
    parent["curation_status"] = parent.apply(curation_status, axis=1)
    ...
    for col in curated_cols:
        if col not in parent.columns:
            parent[col] = ""
    write_csv(parent[curated_cols], "spectrum_level_provenance.csv")

    source_split = (
        parent.groupby(["source_type", "curation_status", "split_main", "class_label"], dropna=False)
        .size()
        .reset_index(name="spectrum_count")
        .sort_values(["source_type", "split_main", "class_label"])
    )
    ...
    included = parent[parent["curation_status"].eq("included_curated")].copy()
    ...
    external = parent[parent["curation_status"].eq("external_sherloc")].copy()

    labels = sorted(set(included["class_label"]) | set(external["class_label"]))
    flow_rows = []
    for label in labels:
        train_count = int((train["class_label"] == label).sum())
        val_count = int((val["class_label"] == label).sum())
        test_count = int((test["class_label"] == label).sum())
        external_count = int((external["class_label"] == label).sum())
        generated = max(0, TARGET_PER_CLASS - train_count) if train_count > 0 else 0
        effective_train = train_count + generated
        flow_rows.append(
            {
                "class_label": label,
                "raw_parent_total": int((parent["class_label"] == label).sum()),
                "included_parent_total": int((included["class_label"] == label).sum()),
                ...
                "augmentation_target_per_class": TARGET_PER_CLASS if train_count > 0 else 0,
    ...
    flow = pd.DataFrame(flow_rows).sort_values("class_label")
    write_csv(flow, "dataset_flow_by_class.csv")

    excluded = parent[parent["curation_status"].eq("excluded_halide_curation")]
    excluded_summary = (
    ...
    sherloc_summary = (
        external.groupby(["sherloc_product_group", "candidate_transfer_role", "class_label", "subtype_label"], dropna=False)
        .size()
        .reset_index(name="spectrum_count")
        .sort_values(["sherloc_product_group", "class_label", "subtype_label"])
    )
    ...
    stage_rows = [
        ...
        {"stage": "curated_earth_train_parent", "spectrum_count": int(len(train)), "description": "Parent spectra used for Earth-domain model training before augmentation."},
        {"stage": "curated_earth_validation_parent", "spectrum_count": int(len(val)), "description": "Parent spectra used for validation before augmentation."},
        {"stage": "curated_earth_test_parent", "spectrum_count": int(len(test)), "description": "Parent spectra used for final Earth-domain test before SHERLOC external evaluation."},
        {"stage": "curated_effective_train_after_augmentation", "spectrum_count": int(flow["effective_train_after_reproducible_augmentation"].sum()), "description": "Effective train set if every included class with train parents is augmented to the target count."},
    ]
    stage_summary = pd.DataFrame(stage_rows)
    write_csv(stage_summary, "dataset_stage_summary.csv")

    workbook = OUT_DIR / "data_inventory_tables.xlsx"
...
4. Halides are excluded from the supervised classification table because pure crystalline halite is weak/non-Raman-active under standard Raman conditions. Halide/chloride detection should be discussed as a separate operational/non-detection or uncertainty problem.
...
- Effective train spectra after reproducible class balancing to {TARGET_PER_CLASS} per class: {int(flow["effective_train_after_reproducible_augmentation"].sum())}

## Files

- `spectrum_level_provenance.csv`: one row per parent spectrum with source, role, QC, hash, and reference fields.
- `dataset_flow_by_class.csv`: raw parent counts, split counts, external SHERLOC counts, and reproducible augmentation counts by class.
...
""",
    ...
    summary = {
        ...
        "curation_status_counts": parent["curation_status"].value_counts().to_dict(),
        "stage_summary_file": str((OUT_DIR / "dataset_stage_summary.csv").relative_to(ROOT)),
        "class_flow_file": str((OUT_DIR / "dataset_flow_by_class.csv").relative_to(ROOT)),
        "workbook": str(workbook.relative_to(ROOT)),
    }
    (OUT_DIR / "data_inventory_summary.json").write_text(
