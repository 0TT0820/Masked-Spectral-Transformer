...
def make_label(row: pd.Series, label_scheme: str) -> str:
    ...
    if label_scheme == "review_ready" and label in PHYLL_LABELS:
...
def load_metadata(label_scheme: str, include_review_required: bool, metadata_file: Path = METADATA_FILE) -> pd.DataFrame:
    ...
    if label_scheme == "review_ready":
        df = df[~df["major_category"].isin(EXCLUDE_LABELS_REVIEW_READY)].copy()
        if not include_review_required:
            # Keep phyllosilicates in review_ready because their labels are harmonized below.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reviewer-oriented Raman model comparison experiment.")
    parser.add_argument("--label-scheme", choices=["review_ready", "original_major"], default="review_ready")
