...
3. The local SHERLOC all-points Raman filenames preserve the PDS SHERLOC product
   identifier, including the sol, spacecraft clock, site/drive, SRLC sequence,
   processing flags, experiment ID, and ACI image number. For example, the
   `w108cgn` segment is interpreted following the SHERLOC RDR SIS as wavelength
   correction (`w`), experiment ID `1`, ACI image number `08`, cosmic-ray
   correction (`c`), gain correction (`g`), and laser normalization (`n`).
   Within each target folder, files were sorted by sol, SRLC sequence, and then
   experiment/image order. The standard-label workbook sheets follow the same
   order, so paired sheets such as `detail_500_1.1` and `detail_500_1.2` are
   mapped to the two corresponding experiment/image products for the same scan
   number.
...
For PDS product-level provenance, including direct PDS CSV/XML label URLs and
raw/intermediate product logical identifiers extracted from the XML labels, see
`sherloc_pds_product_provenance.csv`.
